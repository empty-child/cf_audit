...
def matches(osm_tags, external_tags):
    if 'brand:wikidata' in external_tags:
    ...
    # Used for brand takeovers, where OSM still contains elements with the old name.
    # Since this code uses hard coded values its usage should be well considered.
    import re
    possible_names = ["new brand name", "old brand name", "old name2"]
...
def dataset(fileobj):
    ...
    data_url = '' #TODO 1 # this is the same value as in "download_url"
